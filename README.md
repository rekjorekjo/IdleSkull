...
击败骷髅时会播放全屏 Compose 动画：骷髅从下方冲到中央并放大，**不再播放第一段黑 / 白全屏扩散**；随后直接进入约 3.7 秒的绿 / 红主题火光爆发。火光由深色外焰、主题主色、亮色内焰、热核与多层放射火星叠加，不再只是单色圆形颜料扩散。之后显示带固定裂纹 Overlay 的骷髅和黑 / 白不规则泼墨。`DEFEATED` 整体稍微下移，并把逐字重击节奏拉长到约 3.5 秒。文字结束后追加等级升级画面：新等级位于上方，旧等级位于下方，中间用向上箭头连接，并显示新等级的 HP。裂纹仍是独立绘制层，不永久修改原始骷髅素材。动画只是遮住画面，计时和新等级骷髅的伤害仍在后台继续结算，动画结束后直接显示最新状态。
