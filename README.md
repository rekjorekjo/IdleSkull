...
首批骷髅内容按 8 个主题 × 5 个难度档组成 40 级：宝宝骷髅、小丑骷髅、舞王骷髅、厨子骷髅、武士骷髅、海盗骷髅、蛇王骷髅、恶魔骷髅；每个主题只使用 1 张完整 PNG，E / M / H / I / B（Easy / Medium / Hard / Insane / Boss）仅通过文字区分，因此首批只需要 8 张图片。主题图统一使用白色主体 + 透明背景的 1024×1024 PNG，运行时在深色模式显示白色、浅色模式统一着色为黑色，并保持主体中心与眼窝几何位置一致。
...
资源文件固定放在 `app/src/main/res/drawable-nodpi/skull_theme_*.png`。当前代码包先放入基础骷髅复制品作为占位，后续生成完成后只需覆盖对应 PNG。
