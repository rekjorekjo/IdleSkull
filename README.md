...
眼窝光效不再按“单次摆烂 10 / 20 / 30 分钟”分档，而是直接反映当前骷髅生命状态：HP 越高越亮，开卷会逐渐压暗，摆烂会逐渐恢复。深色模式继续使用红眼，浅色模式使用高亮阴森绿眼。8 个主题分别使用按实际图片眼窝测量的归一化锚点，缩放 PNG 后也不需要重新校准。为了减少无意义的持续重绘，眼光不再做常驻呼吸脉冲。
...
首批骷髅内容按 8 个主题 × 5 个难度档组成 40 级：宝宝骷髅、小丑骷髅、舞王骷髅、厨子骷髅、武士骷髅、海盗骷髅、蛇王骷髅、恶魔骷髅；每个主题只使用 1 张完整 PNG，E / M / H / I / B（Easy / Medium / Hard / Insane / Boss）仅通过文字区分，因此首批只需要 8 张图片。主题图是带透明背景的完整灰度成品，**运行时不再 Tint**，否则骨骼纹理和阴影会被压成纯色剪影。所有主题尽量保持主体中心一致；眼窝位置则按各主题实际图片分别校准，避免强行共用一套坐标。
...
击败骷髅时会播放全屏 Compose 动画：动画触发的第一帧就隐藏首页原本的骷髅图片与眼光（保留原布局占位，避免页面跳动），随后由击败动画中的骷髅从下方冲到中央并放大，因此不会出现两个骷髅叠在一起。中央骷髅随后被真正绘制在其上方的绿 / 红主题**光爆**吞没。光爆约 3.9 秒从中央扩散到屏幕四角，以径向渐变、柔光晕、细亮环、放射光束与发光火星叠加，视觉语言与首页眼睛光效一致，不再使用实心火焰块或泼墨。爆发结束后才重新出现带裂纹 Overlay 的骷髅。裂纹不是另一张静态图片，而是 Compose Canvas 实时绘制的 Path；主裂缝与分支线条保持加粗，并改为约 0.5 秒从中心向外动态生长。裂纹本身统一使用灰黑色与深灰外沿，不再跟随红 / 绿主题光色，便于比较更纯粹的碎裂观感。`DEFEATED` 使用固定左边界，D 首先落在最终单词最左侧，后续字母依次向右重击出现；浅色模式使用深绿色，深色模式使用红色，完整逐字阶段约 2.5 秒。文字结束后追加等级升级画面：下一等级位于上方、旧等级位于下方，中间用向上箭头指向新等级，并显示下一等级的 HP。普通升级同时显示当前 / 下一档难度；当 B 档 Boss 跨入下一主题时，先显示“XXX骷髅 Boss 已被击败”，再显示下一主题 · E 与新等级。Lv.40 暂不做终局特殊结算，继续沿用普通流程。裂纹仍是独立绘制层，不永久修改原始骷髅素材。动画只是遮住画面，计时和新等级骷髅的伤害仍在后台继续结算，动画结束后直接显示最新状态。
