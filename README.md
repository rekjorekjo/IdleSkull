...
击败骷髅时会播放全屏 Compose 动画：骷髅从下方冲到中央并放大，随后被真正绘制在其上方的绿 / 红主题**光爆**吞没。光爆约 3.9 秒从中央扩散到屏幕四角，以径向渐变、柔光晕、细亮环、放射光束与发光火星叠加，视觉语言与首页眼睛光效一致，不再使用实心火焰块或泼墨。爆发结束后才重新出现带固定裂纹 Overlay 的骷髅。`DEFEATED` 使用固定左边界，D 首先落在最终单词最左侧，后续字母依次向右重击出现；浅色模式使用深绿色，深色模式使用红色，完整逐字阶段约 5 秒。文字结束后追加等级升级画面：新等级位于上方，旧等级位于下方，中间用向上箭头连接，并显示新等级的 HP。裂纹仍是独立绘制层，不永久修改原始骷髅素材。动画只是遮住画面，计时和新等级骷髅的伤害仍在后台继续结算，动画结束后直接显示最新状态。
